...
export class Client {
	...
	death(arr) {
		...
		} else if (this.id === this.presidentId) {
			this.publishEvent('killResult', { player });
		} else {
	...
	get playerIsPresident() {
		return this.presidentId === this.id;
	}

	get playerIsChancellor() {
		return this.chancellorId === this.id;
	}

	get voted() {
		this.players.map((player, i) => ({
			player,
			voted: this.votedSet.has(i),
		}));
	}
